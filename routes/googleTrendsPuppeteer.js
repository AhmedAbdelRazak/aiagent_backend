...
const FAST_POTENTIAL_IMAGE_MIN_PER_STORY = 2;
const FAST_POTENTIAL_IMAGE_TARGET_PER_STORY = 4;
const FAST_MIN_FEED_IMAGES_PER_STORY = 2;
...
function buildTrendsCacheKey({
  ...
  return JSON.stringify({
    ...
    potentialImageMode: String(potentialImageMode || ""),
...
async function enrichStoriesWithPotentialImages(
  stories,
  {
    ...
    cbsTopUpTopics = 0,
    enableGoogleImagesTopUp = true,
    minFeedImagesPerStory = MIN_FEED_IMAGES_PER_STORY,
  } = {},
) {
  if (!Array.isArray(stories) || !stories.length) return stories;
  ...
  const minFeedImages = clampInt(
    minFeedImagesPerStory || 0,
    0,
    POTENTIAL_IMAGE_MAX_PER_STORY,
  );
  ...
  log("Potential images enrichment start", {
    ...
    googleImagesTopUp: enableGoogleImagesTopUp,
    minFeedImages,
  });
...
router.get("/google-trends", requireLocalOrInternalKey, async (req, res) => {
  ...
  const potentialImageMode = String(
    req.query.potentialImageMode || req.query.imageMode || "",
  )
    .toLowerCase()
    .trim();
  const useFastPotentialImages = potentialImageMode === "fast";
  ...
  try {
    ...
    if (includePotentialImages) {
      stories = await enrichStoriesWithPotentialImages(stories, {
        ...
        minImagesPerStory: useFastPotentialImages
          ? FAST_POTENTIAL_IMAGE_MIN_PER_STORY
          : POTENTIAL_IMAGE_MIN_PER_STORY,
        targetImagesPerStory: useFastPotentialImages
          ? FAST_POTENTIAL_IMAGE_TARGET_PER_STORY
          : POTENTIAL_IMAGE_MAX_PER_STORY,
        topUpTopics:
          useFastPotentialImages || !isEntertainment
            ? 0
            : POTENTIAL_IMAGE_TOPUP_TOPIC_LIMIT,
        enableVogueFallback: useFastPotentialImages ? false : isEntertainment,
        enableBbcFallback: useFastPotentialImages ? false : true,
        bbcTopUpTopics: useFastPotentialImages
          ? 0
          : POTENTIAL_IMAGE_TOPIC_LIMIT,
        enableCbsFallback: useFastPotentialImages ? false : true,
        cbsTopUpTopics: useFastPotentialImages
          ? 0
          : POTENTIAL_IMAGE_TOPIC_LIMIT,
        enableGoogleImagesTopUp: !useFastPotentialImages,
        minFeedImagesPerStory: useFastPotentialImages
          ? FAST_MIN_FEED_IMAGES_PER_STORY
          : MIN_FEED_IMAGES_PER_STORY,
      });
    } else {
